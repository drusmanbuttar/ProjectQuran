...
self.addEventListener('activate',event=>{event.waitUntil((async()=>{for(const key of await caches.keys())if(key.startsWith('pockett-quran-')&&key!==CACHE)await caches.delete(key);await self.clients.claim()})())});
